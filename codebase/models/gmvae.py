...
class GMVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        m_mixture = m_mixture.expand(batch, -1, -1)
        v_mixture = v_mixture.expand(batch, -1, -1)

        log_normal = ut.log_normal(z, q_m, q_v)
        log_normal_mixture = ut.log_normal_mixture(z, m_mixture, v_mixture)

        kl = (log_normal - log_normal_mixture).mean()
    ...
    def negative_iwae_bound(self, x, iw):
        ...
        # calculate log p(z)
        m_mixture, v_mixture = prior
        m_mixture = m_mixture.expand(batch * iw, -1, -1)
        v_mixture = v_mixture.expand(batch * iw, -1, -1)
        log_prob_z = ut.log_normal_mixture(z, m_mixture, v_mixture).reshape(iw, batch).transpose(1, 0)
